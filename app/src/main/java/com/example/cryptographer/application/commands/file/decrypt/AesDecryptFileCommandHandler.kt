...
/**
 * Command Handler for decrypting files using AES algorithm.
 */
class AesDecryptFileCommandHandler(
    ...
    operator fun invoke(command: AesDecryptFileCommand): Result<DecryptedFileView> {
        return try {
            logger.debug { "AES file decryption: input=${command.inputPath}, output=${command.outputPath}" }
            ...
            logger.error(e) { "AES file decryption failed: ${e.message}" }
            Result.failure(e)
        }
    }
    ...
    private fun readInputBytes(inputPath: String): Result<ByteArray> {
        return fileGateway.readBytes(inputPath)
            .onFailure { error -> logger.error(error) { "AES file decryption failed: read error" } }
    }
    ...
    private fun parseEncryptedInput(inputBytes: ByteArray, algorithm: EncryptionAlgorithm): Result<EncryptedText> {
        return if (inputBytes.size <= IV_LENGTH) {
            ...
            logger.error(error) { "AES file decryption failed: invalid input size" }
    ...
    private fun decryptBytes(encryptedText: EncryptedText, key: EncryptionKey): Result<ByteArray> {
        return aesEncryptionService.decrypt(encryptedText, key)
            .onFailure { error -> logger.error(error) { "AES file decryption failed: decryption error" } }
    }
    ...
    private fun writeOutput(outputPath: String, decryptedBytes: ByteArray): Result<DecryptedFileView> {
        ...
        return if (error != null) {
            logger.error(error) { "AES file decryption failed: write error" }
            Result.failure(error)
        } else {
            logger.info { "AES file decryption successful: output=$outputPath" }
