...
fun ChaCha20DecryptionSection(
    ...
    Card(
        ...
        Column(
            ...
            OutlinedTextField(
                value = uiState.nonceInput,
                onValueChange = onNonceChange,
                modifier = Modifier.fillMaxWidth(),
                label = { Text(stringResource(R.string.nonce_label)) },
                placeholder = { Text(stringResource(R.string.enter_nonce_placeholder)) },
                enabled = !uiState.isLoading && uiState.selectedKey != null,
                shape = RoundedCornerShape(8.dp),
            )

            ChaCha20DecryptionButton(
...
@Composable
private fun ChaCha20DecryptionButton(isLoading: Boolean, enabled: Boolean, onClick: () -> Unit) {
    Button(
        onClick = onClick,
        modifier = Modifier
            .fillMaxWidth()
