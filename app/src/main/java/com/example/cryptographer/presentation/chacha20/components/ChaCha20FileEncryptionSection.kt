...
@Composable
fun ChaCha20FileEncryptionSection(
...
@Composable
private fun ChaCha20FileEncryptionFields(
    uiState: ChaCha20FileUiState,
    onInputChange: (String) -> Unit,
    onOutputChange: (String) -> Unit,
    onPickInput: () -> Unit,
...
@Composable
private fun ChaCha20FileEncryptionResults(uiState: ChaCha20FileUiState, clipboard: Clipboard) {
    ...
    if (uiState.nonceBase64.isNotEmpty()) {
        ChaCha20ResultCard(
            title = stringResource(R.string.nonce_label),
            content = uiState.nonceBase64,
            onCopyClick = {
                scope.launch {
                    clipboard.setClipEntry(
                        ClipData.newPlainText("", uiState.nonceBase64).toClipEntry(),
                    )
                }
            },
...
@Composable
private fun ChaCha20FileEncryptButton(isLoading: Boolean, enabled: Boolean, onClick: () -> Unit) {
    Button(
        onClick = onClick,
        modifier = Modifier
            .fillMaxWidth()
